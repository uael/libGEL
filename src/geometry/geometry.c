...
bool point_inside_plane(t_vec3 point, t_plane p) {
    return vec3dot(vec3sub(point, p.position), p.direction) < 0;
}
...
t_vec3 plane_project(t_plane p, t_vec3 v) {
    return vec3sub(v, vec3mul(p.direction, vec3dot(v, p.direction)));
}

t_vec3 plane_closest(t_plane p, t_vec3 v) {
    return vec3sub(v, vec3mul(p.direction, plane_distance(p, v)));
}

bool point_swept_inside_plane(t_vec3 point, t_vec3 v, t_plane p) {

    float angle = vec3dot(p.direction, v);
    float dist  = vec3dot(p.direction, vec3sub(point, p.position));
...
t_box box_new(float x_min, float x_max, float y_min, float y_max, float z_min, float z_max) {
    ...
    t_box bb;
    bb.top    = plane_new( vec3new(0, y_max,0), vec3new(0, 1,0));
    bb.bottom = plane_new( vec3new(0, y_min,0), vec3new(0,-1,0));
    bb.left   = plane_new( vec3new( x_max,0,0), vec3new( 1,0,0));
    bb.right  = plane_new( vec3new( x_min,0,0), vec3new(-1,0,0));
    bb.front  = plane_new( vec3new(0,0, z_max), vec3new(0,0, 1));
    bb.back   = plane_new( vec3new(0,0, z_min), vec3new(0,0,-1));
    return bb;
...
t_box box_sphere(t_vec3 center, float radius) {
    ...
    t_box bb;
    bb.top    = plane_new(vec3add(center, vec3new(0, radius,0)), vec3new(0, 1,0));
    bb.bottom = plane_new(vec3add(center, vec3new(0,-radius,0)), vec3new(0,-1,0));
    bb.left   = plane_new(vec3add(center, vec3new( radius,0,0)), vec3new( 1,0,0));
    bb.right  = plane_new(vec3add(center, vec3new(-radius,0,0)), vec3new(-1,0,0));
    bb.front  = plane_new(vec3add(center, vec3new(0,0, radius)), vec3new(0,0, 1));
    bb.back   = plane_new(vec3add(center, vec3new(0,0,-radius)), vec3new(0,0,-1));
    return bb;

}

t_box box_invert(t_box b) {
    b.front.direction  = vec3neg(b.front.direction);
    b.back.direction   = vec3neg(b.back.direction);
    b.left.direction   = vec3neg(b.left.direction);
    b.right.direction  = vec3neg(b.right.direction);
    b.top.direction    = vec3neg(b.top.direction);
    b.bottom.direction = vec3neg(b.bottom.direction);
    return b;
}
...
t_frustum frustum_slice(frustum f, float start, float end) {
    t_frustum r;
    r.ntr = vec3add(f.ntr, vec3mul(vec3sub(f.ftr, f.ntr), start));
    r.ftr = vec3add(f.ntr, vec3mul(vec3sub(f.ftr, f.ntr), end  ));

    r.ntl = vec3add(f.ntl, vec3mul(vec3sub(f.ftl, f.ntl), start));
    r.ftl = vec3add(f.ntl, vec3mul(vec3sub(f.ftl, f.ntl), end  ));

    r.nbr = vec3add(f.nbr, vec3mul(vec3sub(f.fbr, f.nbr), start));
    r.fbr = vec3add(f.nbr, vec3mul(vec3sub(f.fbr, f.nbr), end  ));

    r.nbl = vec3add(f.nbl, vec3mul(vec3sub(f.fbl, f.nbl), start));
    r.fbl = vec3add(f.nbl, vec3mul(vec3sub(f.fbl, f.nbl), end  ));
    return r;
}
...
t_frustum frustum_translate(t_frustum f, t_vec3 v) {
    t_frustum r;
    r.ntr = vec3add(f.ntr, v);
    r.ftr = vec3add(f.ftr, v);
    r.ntl = vec3add(f.ntl, v);
    r.ftl = vec3add(f.ftl, v);
    r.nbr = vec3add(f.nbr, v);
    r.fbr = vec3add(f.fbr, v);
    r.nbl = vec3add(f.nbl, v);
    r.fbl = vec3add(f.fbl, v);
    return r;
}
...
t_box frustum_box(t_frustum f) {
    ...
    t_box b;
    b.top     = plane_new(f.ntr, vec3norm(vec3cross(vec3sub(f.ftr, f.ntr), vec3sub(f.ntl, f.ntr))));
    b.bottom  = plane_new(f.nbr, vec3norm(vec3cross(vec3sub(f.nbl, f.nbr), vec3sub(f.fbr, f.nbr))));
    b.left    = plane_new(f.ntl, vec3norm(vec3cross(vec3sub(f.ftl, f.ntl), vec3sub(f.nbl, f.ntl))));
    b.right   = plane_new(f.ntr, vec3norm(vec3cross(vec3sub(f.nbr, f.ntr), vec3sub(f.ftr, f.ntr))));
    b.front   = plane_new(f.ftr, vec3norm(vec3cross(vec3sub(f.ftl, f.ftr), vec3sub(f.fbr, f.ftr))));
    b.back    = plane_new(f.ntr, vec3norm(vec3cross(vec3sub(f.nbr, f.ntr), vec3sub(f.ntl, f.ntr))));
    return b;
}
...
bool sphere_outside_sphere(t_sphere s1, t_sphere s2) {
    ...
    return vec3dist_sqrd(s1.center, s2.center) > rtot * rtot;
}

t_sphere sphere_unit() {
    return sphere_new(vec3zero(), 1);
}

t_sphere sphere_point() {
    return sphere_new(vec3zero(), 0);
}
...
t_sphere sphere_of_box(t_box bb) {
    ...
    float radius = 0;
    radius = max(radius, vec3dist(center, vec3new(x_min, y_min, z_min)));
    radius = max(radius, vec3dist(center, vec3new(x_max, y_min, z_min)));
    radius = max(radius, vec3dist(center, vec3new(x_min, y_max, z_min)));
    radius = max(radius, vec3dist(center, vec3new(x_min, y_min, z_max)));
    radius = max(radius, vec3dist(center, vec3new(x_min, y_max, z_max)));
    radius = max(radius, vec3dist(center, vec3new(x_max, y_max, z_min)));
    radius = max(radius, vec3dist(center, vec3new(x_max, y_min, z_max)));
    radius = max(radius, vec3dist(center, vec3new(x_max, y_max, z_max)));
...
t_sphere sphere_of_frustum(t_frustum f) {
    ...
    s.radius = 0;
    s.radius = max(s.radius, vec3dist(s.center, f.ntr));
    s.radius = max(s.radius, vec3dist(s.center, f.ftr));
    s.radius = max(s.radius, vec3dist(s.center, f.ntl));
    s.radius = max(s.radius, vec3dist(s.center, f.ftl));
    s.radius = max(s.radius, vec3dist(s.center, f.nbr));
    s.radius = max(s.radius, vec3dist(s.center, f.fbr));
    s.radius = max(s.radius, vec3dist(s.center, f.nbl));
    s.radius = max(s.radius, vec3dist(s.center, f.fbl));
...
t_sphere sphere_merge(t_sphere bs1, t_sphere bs2) {
    ...
    t_vec3 dir = vec3sub(bs2.center, bs1.center);
    t_vec3 dirnorm = vec3norm(dir);

    t_vec3 p0 = vec3sub(bs1.center, vec3mul(dirnorm, bs1.radius));
    t_vec3 p1 = vec3add(bs2.center, vec3mul(dirnorm, bs2.radius));

    t_vec3 center = vec3div(vec3add(p0, p1), 2);
    float dist = vec3dist(center, p0);
...
bool point_inside_sphere(t_sphere s, t_vec3 point) {
    return vec3dist(s.center, point) < s.radius;
}
...
bool line_inside_sphere(t_sphere s, t_vec3 start, t_vec3 end) {
    return point_swept_inside_sphere(s, vec3sub(end, start), start);
}

bool line_outside_sphere(t_sphere s, t_vec3 start, t_vec3 end) {
    return point_swept_outside_sphere(s, vec3sub(end, start), start);
}

bool line_intersects_sphere(t_sphere s, t_vec3 start, t_vec3 end) {
    return point_swept_intersects_sphere(s, vec3sub(end, start), start);
}
...
bool sphere_swept_inside_plane(t_sphere s, t_vec3 v, t_plane p) {
    ...
    float angle = vec3dot(p.direction, v);
    float dist  = vec3dot(p.direction, vec3sub(s.center, p.position));
...
bool point_swept_outside_sphere(t_sphere s, t_vec3 v, t_vec3 point) {
    ...
    float sdist = vec3dist_sqrd(point, s.center);

    if (sdist <= s.radius * s.radius) { return false; }

    t_vec3  o = vec3sub(point, s.center);
    float A = vec3dot(v, v);
    float B = 2 * vec3dot(v, o);
    float C = vec3dot(o, o) - (s.radius * s.radius);
...
bool sphere_swept_outside_sphere(t_sphere s1, t_vec3 v, t_sphere s2) {
    ...
    float sdist = vec3dist_sqrd(s1.center, s2.center);
    float rtot = s1.radius + s2.radius;

    if (sdist <= rtot * rtot) { return false; }

    t_vec3  o = vec3sub(s1.center, s2.center);
    float A = vec3dot(v, v);
    float B = 2 * vec3dot(v, o);
    float C = vec3dot(o, o) - (rtot * rtot);
...
bool point_inside_triangle(vec3 p, t_vec3 v0, t_vec3 v1, t_vec3 v2) {
    ...
    t_vec3 d0 = vec3sub(v2, v0);
    t_vec3 d1 = vec3sub(v1, v0);
    t_vec3 d2 = vec3sub(p, v0);

    float dot00 = vec3dot(d0, d0);
    float dot01 = vec3dot(d0, d1);
    float dot02 = vec3dot(d0, d2);
    float dot11 = vec3dot(d1, d1);
    float dot12 = vec3dot(d1, d2);
...
t_ellipsoid ellipsoid_of_sphere(t_sphere s) {
    t_vec3 radiuses = vec3new(s.radius, s.radius, s.radius);
...
bool vertex_equal(t_vertex v1, t_vertex v2) {
    ...
    if(!vec3equ(v1.position, v2.position)) { return false; }
    if(!vec3equ(v1.normal, v2.normal)) { return false; }
...
void vertex_print(t_vertex v) {
    ...
    printf("V(Position: "); vec3print(v.position);
    printf(", Normal: "); vec3print(v.normal);
    printf(", Tangent: "); vec3print(v.tangent);
    printf(", Binormal: "); vec3print(v.binormal);
...
void mesh_generate_tangents(t_mesh* m) {
    ...
    for(int i = 0; i < m->num_verts; i++) {
        m->verticies[i].tangent = vec3zero();
        m->verticies[i].binormal = vec3zero();
    }
    ...
    while( i < m->num_triangles * 3) {
        ...
        v1.tangent = vec3add(face_tangent, v1.tangent);
        v2.tangent = vec3add(face_tangent, v2.tangent);
        v3.tangent = vec3add(face_tangent, v3.tangent);

        v1.binormal = vec3add(face_binormal, v1.binormal);
        v2.binormal = vec3add(face_binormal, v2.binormal);
        v3.binormal = vec3add(face_binormal, v3.binormal);
    ...
    for(int i = 0; i < m->num_verts; i++) {
        m->verticies[i].tangent = vec3norm( m->verticies[i].tangent );
        m->verticies[i].binormal = vec3norm( m->verticies[i].binormal );
    }
...
void mesh_generate_normals(t_mesh* m) {
    ...
    while( i < m->num_triangles * 3) {
        ...
        v1.normal = vec3add(face_normal, v1.normal);
        v2.normal = vec3add(face_normal, v2.normal);
        v3.normal = vec3add(face_normal, v3.normal);
    ...
    for(int i = 0; i < m->num_verts; i++) {
        m->verticies[i].normal = vec3norm( m->verticies[i].normal );
    }
...
void mesh_generate_orthagonal_tangents(t_mesh* m) {
    ...
    while( i < m->num_triangles * 3) {
        ...
        t_vec3 face_tangent = vec3norm( vec3cross(face_binormal_temp, face_normal) );
        t_vec3 face_binormal = vec3norm( vec3cross(face_tangent, face_normal) );
...
void mesh_translate(t_mesh* m, t_vec3 translation) {
    ...
    while(i < m->num_triangles * 3) {
        ...
        m->verticies[t_i1].position = vec3add(m->verticies[t_i1].position, translation);
        m->verticies[t_i2].position = vec3add(m->verticies[t_i2].position, translation);
        m->verticies[t_i3].position = vec3add(m->verticies[t_i3].position, translation);
...
void mesh_scale(t_mesh* m, float scale) {
    ...
    while(i < m->num_triangles * 3) {
        ...
        m->verticies[t_i1].position = vec3mul(m->verticies[t_i1].position, scale);
        m->verticies[t_i2].position = vec3mul(m->verticies[t_i2].position, scale);
        m->verticies[t_i3].position = vec3mul(m->verticies[t_i3].position, scale);
...
t_sphere mesh_bounding_sphere(t_mesh* m) {
    ...
    t_sphere s = sphere_new(vec3zero(), 0);

    for (int i = 0; i < m->num_verts; i++) {
        s.center = vec3add(s.center, m->verticies[i].position);
    }
    s.center = vec3div(s.center, m->num_verts);

    for (int i = 0; i < m->num_verts; i++) {
        s.radius = max(s.radius, vec3dist(s.center, m->verticies[i].position));
    }
...
t_vec3 triangle_binormal(t_vertex vert1, t_vertex vert2, t_vertex vert3) {
    ...
    return vec3norm(sdir);

}

t_vec3 triangle_normal(t_vertex v1, t_vertex v2, t_vertex v3) {
    t_vec3 edge1 = vec3sub(v2.position, v1.position);
    t_vec3 edge2 = vec3sub(v3.position, v1.position);
    t_vec3 normal = vec3cross(edge1, edge2);
...
float triangle_area(t_vertex v1, t_vertex v2, t_vertex v3) {
    ...
    t_vec3 ab = vec3sub(v1.position, v2.position);
    t_vec3 ac = vec3sub(v1.position, v3.position);

    float area = 0.5 * vec3length(vec3cross(ab, ac));
...
t_vec3 triangle_random_position(t_vertex v1, t_vertex v2, t_vertex v3) {
    ...
    a = vec3sub(a, vec3mul(ab , r1) );
    a = vec3sub(a, vec3mul(ac , r2) );
...
t_vertex triangle_random_position_interpolation(t_vertex v1, t_vertex v2, t_vertex v3) {
    ...
    v_pos = v1.position;
    v_pos = vec3sub(v_pos, vec3mul(vec3sub(v1.position, v2.position) , r1) );
    v_pos = vec3sub(v_pos, vec3mul(vec3sub(v1.position, v3.position) , r2) );

    v_norm = v1.normal;
    v_norm = vec3sub(v_norm, vec3mul(vec3sub(v1.normal, v2.normal) , r1) );
    v_norm = vec3sub(v_norm, vec3mul(vec3sub(v1.normal, v3.normal) , r2) );

    v_tang = v1.tangent;
    v_tang = vec3sub(v_tang, vec3mul(vec3sub(v1.tangent, v2.tangent) , r1) );
    v_tang = vec3sub(v_tang, vec3mul(vec3sub(v1.tangent, v3.tangent) , r2) );

    v_binorm = v1.binormal;
    v_binorm = vec3sub(v_binorm, vec3mul(vec3sub(v1.binormal, v2.binormal) , r1) );
    v_binorm = vec3sub(v_binorm, vec3mul(vec3sub(v1.binormal, v3.binormal) , r2) );
